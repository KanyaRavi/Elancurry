...
exports.signup = function (req, res, next) {
  ...
  if(typeof registeringUser.phone == 'undefined' || registeringUser.phone == ''){
    console.log("f");
    res.send(new Response.respondWithData("Failed","Phone number missing"));
    return next();
  }
  ...
  User.findOne({ 'phone': registeringUser.phone }, function(err, existingUser){
     if (err) {
       ...
       res.send(new Response.respondWithData("Failed","User already exists with this mobile number"));
       return next();
     } else {
        User.findOne({'email': registeringUser.email}, function(err, exists){
          if (err) {
            ...
            res.send(new Response.respondWithData("Failed","User exists with this email address"));
            return next();
          } else {
